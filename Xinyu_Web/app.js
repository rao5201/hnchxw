...
window.addEventListener('load', () => {
    ...
    if (token) {
        console.log("从大厅传入 Token:", token);
        localStorage.setItem('teahaixin_token', token);
        // 自动跳转到聊天页面，不显示登录页
        showChatPage(); 
    }
});
...
(function () {
    ...
    const urlUsername = urlParams.get('username'); // 可选：如果大厅传了用户名

    // 如果 URL 中带有 Token
    if (urlToken) {
        console.log('🔐 检测到大厅传入 Token:', urlToken);
        
        // 临时存储 Token，供下面的主程序读取
        // 我们不立即写入 localStorage，而是先标记，防止覆盖用户原本的登录状态
        window.__PREFERRED_TOKEN__ = urlToken;
        window.__PREFERRED_USERNAME__ = urlUsername || '茶友';
        
        // （可选）清理 URL 参数，避免刷新时重复触发
        // 如果你希望刷新后依然保持登录，可以注释掉下面这行
        // window.history.replaceState({}, document.title, window.location.pathname);
    }

    // ===================================================================
    // 2. 原有的主程序逻辑 (保持不变，但修改 loadState)
    // ===================================================================
    'use strict';
    ...
    // ===================================================================
    // 3. 修改 State 管理：优先使用 URL 传入的 Token
    // ===================================================================
    let state = loadState();
    ...
    function loadState() {
        try {
            ...
            let savedState = raw ? JSON.parse(raw) : null;

            // --- 关键逻辑：如果 URL 有 Token，强制使用 URL 的数据 ---
            if (window.__PREFERRED_TOKEN__) {
                console.log('🔄 强制使用大厅登录模式');
                return {
                    ...defaultState(),
                    username: window.__PREFERRED_USERNAME__,
                    token: window.__PREFERRED_TOKEN__,
                    isRegistered: true, // 强制标记为已登录
                    joinDate: savedState?.joinDate || Date.now() // 保留老的注册时间，或者用新的
                };
            }
    ...
    // ... (后续的 API helpers, DOM helpers, Screen logic 保持原样不变) ...
